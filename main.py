...
def obtener_demora_y_verificar(numero_vuelo, origen, destino):
    try:
        with open(ruta_csv_vuelos_nueva, mode='r', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                vuelo = row['numero_vuelo'].strip().upper()
                ...
                if vuelo == numero_vuelo.strip().upper() and dept_arp == origen and arvl_arp == destino:
                    ...
                    try:
                        ...
                        return None
            print(f"No se encontró coincidencia de vuelo, origen y destino para el vuelo {numero_vuelo}")
            return None
...
def pedido_duplicado(rut_dni, numero_vuelo):
    try:
        with open(ruta_pedidos, mode='r', newline='') as file:
            reader = csv.reader(file)
            for row in reader:
                if row[0] == rut_dni and row[1] == numero_vuelo:
                    return row[5]  # Retorna el código de comida si está duplicado
        return None
    except FileNotFoundError:
        return None

def guardar_pedido(rut_dni, numero_vuelo, fecha_hora_salida, origen, destino, codigo_comida):
    with open(ruta_pedidos, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([rut_dni, numero_vuelo, fecha_hora_salida, origen, destino, codigo_comida])
...
def procesar_pedido():
    ...
    if codigo_comida not in codigos_validos:
        return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=codigo_comida, mensaje="Código de comida no válido. Por favor, intente de nuevo.", error=True)

    if not validar_tiempo_salida(fecha_salida):
        return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=codigo_comida, mensaje="El vuelo está a menos de 24 horas de salida. No se puede procesar el pedido de SPML.", error=True)

    demora = obtener_demora_y_verificar(numero_vuelo, origen, destino)
    if demora is None:
        return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=codigo_comida, mensaje="No se pudo obtener la demora del vuelo.", error=True)

    if not validar_ruta_vuelo(demora, origen, destino):
        return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=codigo_comida, mensaje="Ruta no cumple con los requisitos de duración o es nacional. No se puede procesar el pedido de SPML.", error=True)

    comida_existente = pedido_duplicado(rut_dni, numero_vuelo)
    if comida_existente:
        return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=comida_existente, duplicado=True)

    guardar_pedido(rut_dni, numero_vuelo, fecha_salida, origen, destino, codigo_comida)

    return render_template('confirmacion.html', rut_dni=rut_dni, numero_vuelo=numero_vuelo, codigo_comida=codigo_comida)
